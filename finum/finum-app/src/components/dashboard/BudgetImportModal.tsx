...
export function BudgetImportModal({
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Failed to import budgets')
      }

      setResult(data)

      // If successful, reload budgets after a short delay
      if (data.created > 0) {
        setTimeout(() => {
          onImportComplete()
          handleClose()
        ...
        <div className="p-6">
          ...
          <div className="mb-6">
            ...
            <p className="text-xs text-finum-gray-500 mt-2">
              Les colonnes acceptées : name (requis), amount (requis), currency,
              period (monthly/annual/goal), category
            </p>
          </div>
            ...
            <div className="bg-finum-green/10 border border-finum-green/20 rounded-lg p-4">
              <div className="flex items-start gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-finum-green/80 mt-1">
                    {result.created} budget(s) créé(s)
                  </p>
                </div>
              </div>
